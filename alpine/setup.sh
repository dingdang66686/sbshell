    ...
    for pkg in "${dependencies[@]}"; do
        if [[ "$PKG_MANAGER" == "apt" ]]; then
            ...
            if ! rpm -q "$pkg" &>/dev/null; then
                echo -e "${YELLOW}安装依赖: $pkg...${RESET}"
                yum install -y "$pkg" >/dev/null 2>&1 || { echo -e "${RED}❌ 错误：安装 $pkg 失败${RESET}" >&2; exit 1; }
            fi
        fi
    done
...
configure_firewall() {
    ...
    if [[ "$OS_TYPE" == "ubuntu" || "$OS_TYPE" == "debian" ]]; then
        ...
        # 检查并开放用户指定的 SSH 端口
        if ! "$firewall_cmd" status | grep -q "$ssh_port/tcp"; then
            "$firewall_cmd" allow "$ssh_port"/tcp comment 'Allow SSH' >/dev/null || echo -e "${YELLOW}警告: 无法添加 UFW $ssh_port/tcp 规则。${RESET}" >&2
        fi
        # 检查并开放 HTTP 和 HTTPS 端口
        if ! "$firewall_cmd" status | grep -q "80/tcp"; then
            "$firewall_cmd" allow 80/tcp comment 'Allow HTTP' >/dev/null || echo -e "${YELLOW}警告: 无法添加 UFW 80/tcp 规则。${RESET}" >&2
        fi
        if ! "$firewall_cmd" status | grep -q "443/tcp"; then
            "$firewall_cmd" allow 443/tcp comment 'Allow HTTPS' >/dev/null || echo -e "${YELLOW}警告: 无法添加 UFW 443/tcp 规则。${RESET}" >&2
        fi
        ...
        if ! "$firewall_cmd" --query-port=80/tcp >/dev/null 2>&1; then
            "$firewall_cmd" --zone=public --add-port=80/tcp --permanent >/dev/null || echo -e "${YELLOW}警告: 无法添加 Firewalld 80/tcp 规则。${RESET}" >&2
        fi
        if ! "$firewall_cmd" --query-port=443/tcp >/dev/null 2>&1; then
            "$firewall_cmd" --zone=public --add-port=443/tcp --permanent >/dev/null || echo -e "${YELLOW}警告: 无法添加 Firewalld 443/tcp 规则。${RESET}" >&2
        fi
        "$firewall_cmd" --reload >/dev/null || echo -e "${YELLOW}警告: Firewalld 配置重载失败。${RESET}" >&2
