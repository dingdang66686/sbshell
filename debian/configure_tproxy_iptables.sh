#!/bin/bash

# 配置参数
TPROXY_PORT=7895  # 与 sing-box 中定义的一致
ROUTING_MARK=666  # 与 sing-box 中定义的一致
PROXY_FWMARK=1
PROXY_ROUTE_TABLE=100
INTERFACE=$(ip route show default | awk '/default/ {print $5; exit}')

# 保留 IP 地址列表
RESERVED_IP=(
    "127.0.0.0/8"
    "10.0.0.0/8"
    "100.64.0.0/10"
    "169.254.0.0/16"
    "172.16.0.0/12"
    "192.0.0.0/24"
    "192.0.2.0/24"
    "198.51.100.0/24"
    "192.88.99.0/24"
    "192.168.0.0/16"
    "203.0.113.0/24"
    "224.0.0.0/4"
    "240.0.0.0/4"
)

# 自定义绕过的 IP 地址
CUSTOM_BYPASS_IP=("192.168.0.0/16" "10.0.0.0/8")

# 读取当前模式
MODE=$(grep -oP '(?<=^MODE=).*' /etc/sing-box/mode.conf)

# 检查指定路由表是否存在
check_route_exists() {
    ip route show table "$1" >/dev/null 2>&1
    return $?
}

# 创建路由表，如果不存在的话
create_route_table_if_not_exists() {
    if ! check_route_exists "$PROXY_ROUTE_TABLE"; then
        echo "路由表不存在，正在创建..."
        ip route add local default dev "$INTERFACE" table "$PROXY_ROUTE_TABLE" || { echo "创建路由表失败"; exit 1; }
    fi
}

# 等待 FIB 表加载完成
wait_for_fib_table() {
    i=1
    while [ $i -le 10 ]; do
        if ip route show table "$PROXY_ROUTE_TABLE" >/dev/null 2>&1; then
            return 0
        fi
        echo "等待 FIB 表加载中，等待 $i 秒..."
        sleep 1
        i=$((i + 1))
    done
    echo "FIB 表加载失败，超出最大重试次数"
    return 1
}

# 清理现有 sing-box iptables 规则
clearSingboxRules() {
    # 清理 mangle 表
    iptables -t mangle -D PREROUTING -j SINGBOX 2>/dev/null
    iptables -t mangle -D OUTPUT -j SINGBOX_OUTPUT 2>/dev/null
    iptables -t mangle -F SINGBOX 2>/dev/null
    iptables -t mangle -X SINGBOX 2>/dev/null
    iptables -t mangle -F SINGBOX_OUTPUT 2>/dev/null
    iptables -t mangle -X SINGBOX_OUTPUT 2>/dev/null
    
    # 清理路由规则
    ip rule del fwmark $PROXY_FWMARK lookup $PROXY_ROUTE_TABLE 2>/dev/null
    ip route del local default dev "${INTERFACE}" table $PROXY_ROUTE_TABLE 2>/dev/null
    
    echo "清理 sing-box 相关的 iptables 规则"
}

# 仅在 TProxy 模式下应用防火墙规则
if [ "$MODE" = "TProxy" ]; then
    echo "应用 TProxy 模式下的 iptables 防火墙规则..."

    # 创建并确保路由表存在
    create_route_table_if_not_exists

    # 等待 FIB 表加载完成
    if ! wait_for_fib_table; then
        echo "FIB 表准备失败，退出脚本。"
        exit 1
    fi

    # 清理现有规则
    clearSingboxRules

    # 设置 IP 规则和路由
    ip -f inet rule add fwmark $PROXY_FWMARK lookup $PROXY_ROUTE_TABLE
    ip -f inet route add local default dev "${INTERFACE}" table $PROXY_ROUTE_TABLE
    sysctl -w net.ipv4.ip_forward=1 > /dev/null

    # 创建 SINGBOX 链
    iptables -t mangle -N SINGBOX
    iptables -t mangle -N SINGBOX_OUTPUT

    # PREROUTING 链规则
    # DNS 请求重定向到本地 TProxy 端口
    iptables -t mangle -A SINGBOX -p tcp --dport 53 -j TPROXY --on-port $TPROXY_PORT --tproxy-mark $PROXY_FWMARK
    iptables -t mangle -A SINGBOX -p udp --dport 53 -j TPROXY --on-port $TPROXY_PORT --tproxy-mark $PROXY_FWMARK

    # 自定义绕过地址
    for ip in "${CUSTOM_BYPASS_IP[@]}"; do
        iptables -t mangle -A SINGBOX -d "$ip" -j RETURN
    done

    # 保留地址绕过
    for ip in "${RESERVED_IP[@]}"; do
        iptables -t mangle -A SINGBOX -d "$ip" -j RETURN
    done

    # 重定向剩余流量到 TProxy 端口并设置标记
    iptables -t mangle -A SINGBOX -p tcp -j TPROXY --on-port $TPROXY_PORT --tproxy-mark $PROXY_FWMARK
    iptables -t mangle -A SINGBOX -p udp -j TPROXY --on-port $TPROXY_PORT --tproxy-mark $PROXY_FWMARK

    # 应用到 PREROUTING 链
    iptables -t mangle -A PREROUTING -j SINGBOX

    # OUTPUT 链规则
    # 本地 sing-box 发出的流量绕过
    iptables -t mangle -A SINGBOX_OUTPUT -m mark --mark $ROUTING_MARK -j RETURN

    # DNS 请求标记
    iptables -t mangle -A SINGBOX_OUTPUT -p tcp --dport 53 -j MARK --set-mark $PROXY_FWMARK
    iptables -t mangle -A SINGBOX_OUTPUT -p udp --dport 53 -j MARK --set-mark $PROXY_FWMARK

    # 绕过 NBNS 流量
    iptables -t mangle -A SINGBOX_OUTPUT -p udp --dport 137 -j RETURN
    iptables -t mangle -A SINGBOX_OUTPUT -p udp --dport 138 -j RETURN
    iptables -t mangle -A SINGBOX_OUTPUT -p tcp --dport 139 -j RETURN

    # 自定义绕过地址
    for ip in "${CUSTOM_BYPASS_IP[@]}"; do
        iptables -t mangle -A SINGBOX_OUTPUT -d "$ip" -j RETURN
    done

    # 保留地址绕过
    for ip in "${RESERVED_IP[@]}"; do
        iptables -t mangle -A SINGBOX_OUTPUT -d "$ip" -j RETURN
    done

    # 标记并重定向剩余流量
    iptables -t mangle -A SINGBOX_OUTPUT -p tcp -j MARK --set-mark $PROXY_FWMARK
    iptables -t mangle -A SINGBOX_OUTPUT -p udp -j MARK --set-mark $PROXY_FWMARK

    # 应用到 OUTPUT 链
    iptables -t mangle -A OUTPUT -j SINGBOX_OUTPUT

    # 持久化防火墙规则
    if command -v iptables-save &> /dev/null; then
        iptables-save > /etc/iptables/rules.v4 2>/dev/null || iptables-save > /etc/iptables.rules 2>/dev/null
    fi

    echo "TProxy 模式的 iptables 防火墙规则已应用。"
else
    echo "当前模式为 TUN 模式，不需要应用防火墙规则。" >/dev/null 2>&1
fi
